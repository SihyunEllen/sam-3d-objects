...
from sam3d_objects.pipeline.inference_pipeline_pointmap_new import InferencePipelinePointMap
...
class Inference:
    ...
    def __init__(self, config_file: str, compile: bool = False):
        ...
        config._target_ = "sam3d_objects.pipeline.inference_pipeline_pointmap_new.InferencePipelinePointMap"
...
def render_video(
    ...
    **kwargs,
    ...
    if torch.cuda.is_available():
        sample = _move_gaussian_to_device(sample, torch.device("cuda"))
...
def _move_gaussian_to_device(scene_gs, device):
    for key, value in vars(scene_gs).items():
        if isinstance(value, torch.Tensor):
            setattr(scene_gs, key, value.to(device))
    scene_gs.device = str(device)
    return scene_gs
